...
require('lazy').setup({
  ...
  { -- AI completion powered by Codeium (free)
    'monkoose/neocodeium',
    event = 'VeryLazy',
    config = function()
      local neocodeium = require 'neocodeium'
      neocodeium.setup {
        -- Show suggestion count in the number column
        show_label = true,
        -- No debounce for instant suggestions
        debounce = false,
        ...
        -- Don't show suggestions when blink.cmp menu is visible
        filter = function()
      ...
      -- Keymaps for neocodeium (minimal setup)
      vim.keymap.set('i', '<C-l>', function()
        neocodeium.accept()
      end, { desc = 'Accept AI suggestion' })
      vim.keymap.set('i', '<C-]>', function()
        neocodeium.cycle_or_complete()
      end, { desc = 'Cycle AI suggestion' })

      -- Integration with blink.cmp - clear suggestions when completion menu opens
  ...
  { -- Winbar with breadcrumb navigation
    ...
    config = function(_, opts)
      -- Set highlights BEFORE setup to ensure they're applied
      vim.api.nvim_create_autocmd('ColorScheme', {
        callback = function()
          local normal_bg = vim.api.nvim_get_hl(0, { name = 'Normal' }).bg
          local normal_fg = vim.api.nvim_get_hl(0, { name = 'Normal' }).fg
          vim.api.nvim_set_hl(0, 'WinBar', { bg = normal_bg, fg = normal_fg })
          vim.api.nvim_set_hl(0, 'WinBarNC', { bg = normal_bg, fg = normal_fg })
        end,
      })
